...
class Distributor {
  static safelyResolveNestedDomainObject(domainObject, partitionKey) {
    ...
    if (!Object.prototype.hasOwnProperty.call(domainObject.domains, partitionKey)) {
      return undefined;
    }
  ...
  constructor(options) {
    ...
    if (typeof wrappedReducer !== 'function') {
      this.handleError(`invalid option reducer is specified "${wrappedReducer}", expected function`);
    }
  ...
  reduce(prevRootState = {}, action) {
    ...
    while (parts.length) {
      ...
      nextWindow.domains = {
        ...
        [partitionKey]: {
          ...Distributor.safelyResolveNestedDomainObject(prevWindow, partitionKey),
        },
      };
      prevWindow = Distributor.safelyResolveNestedDomainObject(prevWindow, partitionKey);
      nextWindow = nextWindow.domains[partitionKey];
    }
    nextWindow.domainState = this.wrappedReducer(nextWindow.domainState, action);
  ...
  selectDomainObject(rootState, domain) {
    ...
    return this
      .resolveDomainPartitions(domain)
      .reduce(Distributor.safelyResolveNestedDomainObject, rootState);
  }

  selectDomainState(rootState, domain) {
    const {
      domainState,
    } = this.selectDomainObject(rootState, domain) || {};
    return domainState;
  }

  selectNestedDomainStates(rootState, domain) {
    const baseState = domain ? this.selectDomainState(rootState, domain) : rootState;
    if (!baseState || !baseState.domains) {
      return [];
    }
    return [].concat(...Object.keys(baseState.domains).map((
      partitionKey => this.selectDomainStates((
...
const distributeReducer = (options) => {
  ...
  return {
    ...
    selectDomainState: (state, domain) => distributor.selectDomainState(state, domain),
    selectDomainStates: (state, domain) => distributor.selectDomainStates(state, domain),
  };
};
