...
describe('distribution', () => {
  ...
  it('should consider requestDomain as part of path', () => {
    ...
    const action = {
      type: TEST_ACTION_TYPE, meta: { domain },
    };
    const reducer = distributeReducerByDomain({
      reducer: () => (
        { isReduced: true }
  ...
  it('should perform over previous state', () => {
    ...
    const prevState = { domains: { [domain]: { domainState: prevDomainState } } };
    const action = {
      type: TEST_ACTION_TYPE, meta: { domain },
    };
    const reducer = distributeReducerByDomain({
      reducer: state => (
        { isReduced: true, prevState: state }
      ),
    }).reduce;
    expect(reducer(prevState, action).domains[domain].domainState.prevState).toBe(prevDomainState);
  });
