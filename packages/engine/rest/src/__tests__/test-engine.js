...
describe('configuration', () => {
  ...
  describe('request endpoint', () => {
    ...
    it('should resolve endpoint with request params, if endpoint option is function', () => {
      ...
      expect(global.fetch.mock.calls[0][0]).toBe(`/${requestConsts.METHOD.GET}/test`);
    });
  });
  ...
  describe('plugins', () => {
    ...
    let pluginB;
    beforeEach(() => {
      pluginA = jest.fn(next => params => next({
        ...params,
        lastPluginCalled: 'A',
        isPluginACalled: true,
      }));
      pluginB = jest.fn(next => params => next({
        ...params,
        lastPluginCalled: 'B',
...
describe('fetch presets', () => {
  it('should carry preset params', () => {
    ...
    const requestParams = global.fetch.mock.calls[0][1];
  ...
  it('should chain preset calls', () => {
    ...
    const requestEngine = createRequestEngine();
    const requestPreset = requestEngine.preset({ presetParam1 }).preset({ presetParam2 });
    requestPreset.request({ endpoint: '/' });
    const requestParams = global.fetch.mock.calls[0][1];
    expect(requestParams).toEqual(expect.objectContaining({ presetParam1, presetParam2 }));
  });
  it('should shallow merge with request params', () => {
    const presetParam = 'presetParam';
    const sharedParamFromPreset = { value: 'sharedParamFromPreset', shouldBeDeeplyMerged: false };
    const sharedParamFromRequest = { value: 'sharedParamFromRequest' };
    const requestParam = 'requestParam';
    const requestEngine = createRequestEngine();
    const requestPreset = requestEngine.preset({ presetParam, sharedParam: sharedParamFromPreset });
    requestPreset.request({ requestParam, sharedParam: sharedParamFromRequest, endpoint: '/' });
    const requestParams = global.fetch.mock.calls[0][1];
  ...
  it('should especially shallow merge preset headers and request headers configurations', () => {
    ...
    const requestHeaders = global.fetch.mock.calls[0][1];
  ...
  it('should not shallow merge other deep objects in configuration configurations', () => {
    ...
    const requestDeepParams = global.fetch.mock.calls[0][1];
