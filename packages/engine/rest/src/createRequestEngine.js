...
const mergeOptions = (...sources) => {
  const mergedHeaders = Object.assign({}, ...sources.map(({ headers = {} } = {}) => headers));
  return Object.assign({}, ...sources, { headers: mergedHeaders });
};
const compose = (...fns) => params => fns.reduceRight((composed, fn) => fn(composed), params);
const defaultFetchAdapter = next => ({ endpoint, ...restOptions }) => next(endpoint, restOptions);
...
class RequestEngine {
  ...
  preset(presetOptions) {
    ...
    return new RequestEngine({
      ...this.configuration,
      presetOptions: currentOptions ? mergeOptions(currentOptions, presetOptions) : presetOptions,
    });
  }
